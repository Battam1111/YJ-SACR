...
def plot_data(dfs: List[pd.DataFrame], std_dfs: List[pd.DataFrame], labels: List[str], columns: List[str], xlabel: str, ylabel: str, output_file: str, window_size: int = 100, plot_std: bool = True):
    ...
    label_color_map = {
        "Original - uniform_v1": "blue",
        "RefineT - uniform_v1": "green",
        "Original - deterministic": "red",
        "RefineT - deterministic": "purple"
    }

    def plot_curve(suffix: str, smooth: bool = False, sliding: bool = False):
        """内部函数，用于绘制曲线。"""
        ...
        for df, std_df, label in zip(dfs, std_dfs, labels):
            ...
            for column in available_columns:
                ...
                plt.plot(df.index, data, label=f"{label} - {column} {'(Smoothed)' if smooth else ''}{' (Sliding)' if sliding else ''}", color=color)
                if plot_std:
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(output_file.replace('.pdf', f'{suffix}.pdf'), dpi=300)  # 修改为保存为PDF格式
        plt.close()
...
def plot_single_experiment(csv_files_TR1: List[str], csv_files_TL1: List[str], csv_files_TR2: List[str], csv_files_TL2: List[str], output_dir: str, af_param: str, seed: str, window_size: int = 100):
    ...
    # 设置标题
    reward_title = f"Average Reward vs Epoch (AF={af_param}, Seed={seed})"
    length_title = f"Episode Length vs Epoch (AF={af_param}, Seed={seed})"
...
def plot_multiple_experiments(csv_files_TR1: List[str], csv_files_TL1: List[str], csv_files_TR2: List[str], csv_files_TL2: List[str], output_dir: str, af_param: str, seeds: List[str], window_size: int = 100):
    ...
    # 设置标题
    reward_title = f"Average Reward vs Epoch (AF={af_param}) - Seeds: {seeds_str}"
    length_title = f"Episode Length vs Epoch (AF={af_param}) - Seeds: {seeds_str}"
...
output_root_dir = 'mujoco-benchmark/PlotSet-FULL-NoRD-debug'

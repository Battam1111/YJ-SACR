...
class BasePolicy(ABC, nn.Module):
    ...
    * :meth:`~tianshou.policy.BasePolicy.update`: the main interface for training, \
    ...
    def learn_actor(self, batch, **kwargs):
        """只更新 Actor 的逻辑。"""
        # 使用 critic1 所在的设备
        device = next(self.critic1.parameters()).device

        # 确保 obs 是 PyTorch 张量，并转换到正确的设备
        obs = torch.as_tensor(batch.obs, device=device, dtype=torch.float32)
        
        # print(f"Obs 形状: {obs.shape}, 数据类型: {type(obs)}")

        # 调用 Actor 网络
        actions, _ = self.actor(obs)[0]  # 获取 Actor 输出的动作
        # 通过 Critic 计算 Actor 的损失
    ...
    def update(self, sample_size: int, buffer: Optional[ReplayBuffer],
           train_actor_only=False, **kwargs: Any) -> Dict[str, Any]:
        ...
        if train_actor_only:
            # 如果只训练 Actor，跳过 Critic 的更新逻辑
            result = self.learn_actor(batch, **kwargs)
        else:
            # 默认行为：更新 Actor 和 Critic

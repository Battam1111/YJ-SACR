...
class Collector(object):
    ...
    def collect(
        ...
        while True:
            ...
            #     # 获取当前epoch编号
            #     epoch_num = self.policy.epoch
            #     directory = 'YJ-SACR/DataSetCollection'
            #     filename = f'{directory}/obs_rewards_epoch_{epoch_num}.csv'
            ...
            #     # 将obs_next和rew与env_id一起写入指定路径的.csv文件中，各一列，每一行代表一个时间步，列名为env_id, obs, rew
            #     with open(filename, mode='a', newline='') as file:
            #         writer = csv.writer(file)
                    
            #         # 如果文件不存在，则写入列名
            #         if not file_exists:
            #             writer.writerow(['env_id', 'obs', 'reward'])  # 增加env_id字段
                    
            #         # 写入数据行
            #         for env_id, (obs, r) in zip(ready_env_ids, zip(obs_next, rew)):
            #             writer.writerow([env_id, obs, r])  # 记录env_id, obs_next, 和rew

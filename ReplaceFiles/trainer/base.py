...
class BaseTrainer(ABC):
    ...
    def __init__(
        ...
        self.csv_file_TR = f'mujoco-benchmark/data/{self.test_collector.task if not self.test_collector.policy.RefineTrain_Num > -1 else (self.test_collector.task + "-RefineT")}/AF{self.test_collector.policy._alpha if not self.test_collector.policy._is_auto_alpha else "Auto"}/TestRew/Seed{self.test_collector.seed}_{timestamp}.csv'
        self.csv_file_TL = f'mujoco-benchmark/data/{self.test_collector.task if not self.test_collector.policy.RefineTrain_Num > -1 else (self.test_collector.task + "-RefineT")}/AF{self.test_collector.policy._alpha if not self.test_collector.policy._is_auto_alpha else "Auto"}/TestLen/Seed{self.test_collector.seed}_{timestamp}.csv'
    ...
    def reset(self) -> None:
        ...
        if self.test_collector is not None:
            ...
            # 确保我们从'deterministic'方法获取结果
            deterministic_result = test_results.get("deterministic", {})
            self.best_epoch = self.start_epoch
            self.best_reward = deterministic_result.get("rew", 0.0)
            self.best_reward_std = deterministic_result.get("rew_std", 0.0)
    ...
    def test_step(self) -> Tuple[Dict[str, Any], bool]:
        ...
        test_stats = test_episode(
            self.policy, self.test_collector, self.test_fn, self.epoch,
            self.episode_per_test, self.logger, self.env_step, self.reward_metric
        )

        for method, test_result in test_stats.items():
            len, rew, rew_std = test_result["len"], test_result["rew"], test_result["rew_std"]
            ...
            if self.verbose:
                print(
                    f"Epoch #{self.epoch} ({method}): test_reward: {rew:.6f} ± {rew_std:.6f},"
        ...
        # 保存评估结果到CSV文件
        self.save_test_rewards_to_csv(self.epoch, test_stats)
        self.save_test_lens_to_csv(self.epoch, test_stats)

        if not self.is_run:
            test_stat = {
                "test_reward": {method: stats["rew"] for method, stats in test_stats.items()},
                "test_reward_std": {method: stats["rew_std"] for method, stats in test_stats.items()},
                "best_reward": self.best_reward,
                "best_reward_std": self.best_reward_std,
                "best_epoch": self.best_epoch,
                "test_len": {method: stats["len"] for method, stats in test_stats.items()}
            }
        else:
            test_stat = {}
    ...
    def save_test_rewards_to_csv(self, epoch: int, test_stats: Dict[str, Dict[str, Any]]) -> None:
        """Save test rewards to CSV file."""
        # 检查文件夹是否存在，如果不存在则创建
        os.makedirs(os.path.dirname(self.csv_file_TR), exist_ok=True)

        file_exists = os.path.isfile(self.csv_file_TR)
        with open(self.csv_file_TR, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            if not file_exists:
                # 写入CSV文件表头
                writer.writerow(['epoch'] + list(test_stats.keys()))
            # 写入每个epoch的评估结果
            row = [epoch] + [test_stats[method]['rew'] for method in test_stats.keys()]
            writer.writerow(row)


    def save_test_lens_to_csv(self, epoch: int, test_stats: Dict[str, Dict[str, Any]]) -> None:
        """Save test rewards to CSV file."""
        # 检查文件夹是否存在，如果不存在则创建
        os.makedirs(os.path.dirname(self.csv_file_TL), exist_ok=True)

        file_exists = os.path.isfile(self.csv_file_TL)
        with open(self.csv_file_TL, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            if not file_exists:
                # 写入CSV文件表头
                writer.writerow(['epoch'] + list(test_stats.keys()))
            # 写入每个epoch的评估结果
            row = [epoch] + [test_stats[method]['len'] for method in test_stats.keys()]
            writer.writerow(row)
    ...
    def train_step(self) -> Tuple[Dict[str, Any], Dict[str, Any], bool]:
        ...
        if result["n/ep"] > 0:
            if self.test_in_train and self.stop_fn and self.stop_fn(result["rew"]):
                ...
                if self.stop_fn(test_result["deterministic"]["rew"]):
                    stop_fn_flag = True
                    self.best_reward = test_result["deterministic"]["rew"]
                    self.best_reward_std = test_result["deterministic"]["rew_std"]
                else:
                    self.policy.train()
